import {add_css as a,add_body as b} from'/js/src/utilities.min.js';a('ul>li>a{color: black} button#down{ background-color: #00FF40;a{color: black; }}');let c=document.createElement('div');c.style.padding='0px 12px';c.innerHTML=`
<br>
   <div class="card" style="width: 18rem">
        <div class="card-header" id="this">Also Try These Question</div>
        <ul class="list-group list-group-flush">
          <li class="list-group-item">
            <a href="fcfs.html">practice first come first serve questions</a>
          </li>
          <li class="list-group-item">
            <a href="sjf1.html">practice shortest job first(SJF) question</a>
          </li>
           <li class="list-group-item">
            <a href="sjf2.html">practice Shortest Remaining Time First (SRTF) question</a>
          </li>
          <li class="list-group-item">
            <a href="mlfqs.html">practice Multilevel feedback queue scheduling question</a>
          </li>
          <li class="list-group-item">
            <a href="MLQS.html">practice Multilevel queue scheduling question</a>
          </li>
          <li class="list-group-item">
            <a href="ps1.html">practice priority scheduling question</a>
          </li>
          <li class="list-group-item">
            <a href="ps2.html">practice peemptive priority scheduling question</a>
          </li>
              <li class="list-group-item">
            <a href="RR1.html">practice Round Robin scheduling question</a>
          </li>
        </ul>
      </div>
      <br>
`;let d=document.getElementById('question1');d.appendChild(c);let e=document.querySelector('div#this.card-header');e.style.color='white';e.style.backgroundColor='#0A0AFF';e.style.fontWeight='bold';let f=document.querySelectorAll('#question1 > div:nth-child(3) > div > ul>li');for(let A=0;A<f.length;A++)f[A].style.backgroundColor='#1AFF53';let g=document.createElement('div');g.innerHTML=`
    <div class="modal fade" id="myModal" tabindex="-1" aria-labelledby="exampleModalLabel">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">Also try these question</h5>
                    <button type="button" id="btn_closeder" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                       <ul class="list-group list-group-flush">
          <li class="list-group-item">
            <a href="fcfs.html">practice first come first serve questions</a>
          </li>
          <li class="list-group-item">
            <a href="sjf1.html">practice shortest job first(SJF) question</a>
          </li>
           <li class="list-group-item">
            <a href="sjf2.html">practice Shortest Remaining Time First (SRTF) question</a>
          </li>
          <li class="list-group-item">
            <a href="mlfqs.html">practice Multilevel feedback queue scheduling question</a>
          </li>
          <li class="list-group-item">
            <a href="MLQS.html">practice Multilevel queue scheduling question</a>
          </li>
          <li class="list-group-item">
            <a href="ps1.html">practice priority scheduling question</a>
          </li>
          <li class="list-group-item">
            <a href="ps2.html">practice peemptive priority scheduling question</a>
          </li>
              <li class="list-group-item">
            <a href="RR1.html">practice Round Robin scheduling question</a>
          </li>
        </ul>
                </div>
            </div>
        </div>
    </div>
`;b(g);let h=document.createElement('script');h.innerHTML=` 
        function openModal() {
            var myModal = new bootstrap.Modal(document.getElementById('myModal'));
            myModal.show();
        }
            setTimeout(openModal, 10000); // 10 minutes interval
            document.getElementById('btn_closeder').addEventListener('click',() => setTimeout(openModal, 12 * 60 * 1000));
   `;b(h);
